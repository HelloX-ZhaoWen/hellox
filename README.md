...
| Mac 灵动岛 | 屏幕顶部的应用与工具入口；展开后切换运行中应用，一键启动全部 17 项 HelloX 功能，支持应用置顶、功能排序和全屏空间显示 |
| Markdown 工作台 | 多文档标签、目录导航、阅读与编辑，以及 HTML / PDF 导出 |
| 脑图 | 独立的原生离线脑图画布，支持一级分支自动左右布局、子主题快捷创建及 PNG / PDF / SVG 导出 |
...
- **HelloX 工具**：通过图标网格直接启动全部 17 项功能，覆盖截图、录屏、水印、OCR、翻译、脑图模块、CSV 转 Excel、Base64 转换、二维码识别、取色、密码生成和 Markdown。
- **快捷键提示**：已配置快捷键的工具会在名称下方显示按键组合，鼠标操作与键盘调用均可使用。
- **显示与排序**：从面板右下角进入设置，管理灵动岛开关、全屏空间显示和功能顺序。

![HelloX 灵动岛展开面板：运行中应用与 HelloX 工具入口](docs/screenshots/dynamic-island-expanded.png)
...
创建左右布局脑图，使用快捷键添加主题、拖动调整层级，并导出 PNG、PDF 或 SVG。多行节点支持 `Shift + Enter` 换行并自动增高。

![HelloX 脑图：左右自动布局、多级主题与多行节点](docs/screenshots/hellox-1.0.6-mind-map.png)

## 安装

**系统要求：macOS 15 或更高版本，Apple Silicon 或 Intel Mac。**

当前版本：**v1.0.6（构建 103）**。查看[更新说明](docs/releases/v1.0.6.md)，或下载 [DMG 安装镜像](https://github.com/HelloX-ZhaoWen/hellox/releases/download/v1.0.6/HelloX-1.0.6.dmg) / [PKG 安装包](https://github.com/HelloX-ZhaoWen/hellox/releases/download/v1.0.6/HelloX-1.0.6.pkg)。
...
- **脑图**：从菜单栏或灵动岛打开脑图模块。新建脑图仅预置中心主题与 4 个一级主题，默认使用统一蓝色系，一级分支自动分布到中心主题两侧，使用 `Tab` 添加子主题、`Enter` 添加同级主题；双击节点编辑文字时，`Shift + Enter` 换行、`Enter` 完成编辑，节点随文字自动增高并调整分支间距，点击其他位置也可结束编辑；选中主题后拖动时显示半透明预览、蓝色插入标记，松手后调整顺序或跨分支移动，其他主题自动让位，拖到空白处取消移动；新建子主题按层级设置默认尺寸（一级 166 × 56、二级 132 × 40、三级及以下 118 × 34），同一父主题下排序保留手动尺寸，移入其他父主题后整棵分支按新层级恢复默认尺寸；右侧样式面板可展开或收起，文档可保存为 `.hxdiagram` 并导出 4 倍分辨率 PNG、PDF 或 SVG。
